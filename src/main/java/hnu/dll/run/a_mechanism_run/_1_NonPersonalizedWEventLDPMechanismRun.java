...
public class _1_NonPersonalizedWEventLDPMechanismRun {
    /**
     * dataList 的外层List代表timestamp，内层list代表user
     */

    public static ExperimentResult runBatch(LPMechanism scheme, Integer batchID, List<List<Integer>> batchDataList, List<Map<Integer, Double>> rawPublicationBatchList) {
        ...
        List<Double> rawPublicationDataList;
        List<Double> publicationDataList;
        ...
        for (int i = 0; i < timeBatchSize; i++) {
            rawPublicationDataList = BasicUtils.toSortedListByKeys(rawPublicationBatchList.get(i), domainIndexList, 0D);
            publicationDataList = BasicUtils.toSortedListByKeys(publicationList.get(i), domainIndexList, 0D);
            batchTotalNormSquare += Measurement.get2NormSquareError(rawPublicationDataList, publicationDataList);
            batchTotalDivergence += Measurement.getJSDivergence(rawPublicationDataList, publicationDataList);
        }
