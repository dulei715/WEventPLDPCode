...
public class MechanismOnTrajectoryDatasetTest {
...
//            Thread.sleep(4000);
    ...
    @Test
    public void mechanismTest5() throws InterruptedException {
        Integer timeSlotSize = timeStampDataFiles.length;
        File file;
        List<Integer> dataList;
...
//            Thread.sleep(4000);
    ...
    @Test
    public void mechanismBasicTest() throws InterruptedException {
        Integer timeSlotSize = timeStampDataFiles.length;
        File file;
        List<Integer> dataList;
...
//        PLDPPopulationAbsorptionPlus pLPAPlus = new PLDPPopulationAbsorptionPlus(dataType, personalizedPrivacyBudgetList, personalizedWindowSizeList, random);
        ...
        List<Double> budgetChangeList = Arrays.asList(1.5);
        ...
        for (int i = 0; i < timeSlotSize; i++) {
            ...
            for (int j = 0; j < lpdMechanismList.size(); j++) {
                ...
                List<Double> tempStatisticList = BasicUtils.toSortedListByKeys(tempStatistic, this.domainIndexList, 0D);
...
//        List<Double> budgetChangeList = Arrays.asList(BasicArrayUtil.getIncreaseDoubleNumberArray(0.2, 0.6, 2.6));
        ...
        for (int i = 0; i < timeSlotSize; i++) {
            ...
            for (int j = 0; j < lpdMechanismList.size(); j++) {
                ...
                Map<Integer, Double> tempStatistic = lpd.updateNextPublicationResult(dataList).getValue();
                List<Double> tempStatisticList = BasicUtils.toSortedListByKeys(tempStatistic, this.domainIndexList, 0D);
//                Double tempError = Measurement.getMRE(tempStatisticList, nonPrivacyResultList);
...
//            Thread.sleep(5000);
    ...
    public void mechanismTest1() throws InterruptedException {
        ...
        List<Double> finalError = new ArrayList<>(budgetChangeList.size());
        for (Double budget : budgetChangeList) {
            lpaMechanismList.add(new LDPPopulationAbsorption(dataType, budget, windowSizeDefault, trajectoryUserSize, random));
            finalError.add(0D);
        }
        ...
        for (int i = 0; i < timeSlotSize; i++) {
            ...
            Map<Integer, Double> nonPrivacyMap = nonPrivacyMechanism.updateNextPublicationResult(dataList).getValue();
            List<Double> nonPrivacyResultList = BasicUtils.toSortedListByKeys(nonPrivacyMap, this.domainIndexList, 0D);
            for (int j = 0; j < lpaMechanismList.size(); j++) {
                LDPPopulationAbsorption lpa = lpaMechanismList.get(j);
                Map<Integer, Double> tempStatistic = lpa.updateNextPublicationResult(dataList).getValue();
                List<Double> tempStatisticList = BasicUtils.toSortedListByKeys(tempStatistic, this.domainIndexList, 0D);
                Double tempError = Measurement.get2NormSquareError(tempStatisticList, nonPrivacyResultList);
                finalError.set(j, finalError.get(j) + tempError);
            }
